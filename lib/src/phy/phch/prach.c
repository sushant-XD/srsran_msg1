                           ...
                           srsran_tdd_config_t* tdd_config)
{
  ...
  if (p != NULL && N_ifft_ul < 2049 && cfg->config_idx < 64 && cfg->root_seq_idx < MAX_ROOTS) {
    ...
    DEBUG("PRACH set_cell_ completed - MSG1 params: enabled=%d, num_preambles=%d, max_idx=%d, power=%.2f",
          p->msg1_enabled,
          p->msg1_num_preambles,
          p->msg1_max_preamble_index,
          p->msg1_preamble_power);
...
int srsran_prach_gen_all(srsran_prach_t* p, uint32_t freq_offset, cf_t* signal)
{
  ...
  if (p != NULL && signal != NULL) {
    ...
    for (uint32_t i = 0; i < num_preambles; i++) {
      uint32_t idx = (random_seed + i * (max_index / num_preambles)) % max_index;
    ...
    if (p->msg1_ramping_step > 0) {
      ...
      if (total_ramp_db > p->msg1_max_ramping_db) {
        DEBUG("MSG1 Generation - Ramping capped at max: %.2f dB (was %.2f dB)", p->msg1_max_ramping_db, total_ramp_db);
      ...
      DEBUG("MSG1 Generation - Ramping factor: %.4f (linear), final power: %.2f", ramp_factor, power_ctrl);
...
int srsran_prach_gen_cancellation(srsran_prach_t* p, uint32_t freq_offset, cf_t* signal)
{
  ...
  if (p != NULL && signal != NULL) {
    ...
    // --- GENERATE THE ANTI-PREAMBLE ---
    // We want to cancel Preamble Index 0
    uint32_t target_preamble = 0;

    cf_t* sequence_data = get_precoded_dft(p, target_preamble);

    // COMPLEX SUBTRACTION / INVERSION
    // Instead of adding (+), we set it to the negative value.
    // We multiply by a gain factor (e.g., 1.0 or higher) to try to match/exceed the victim's power.
    float gain = 5.0f; // Adjust this based on how close you are relative to the victim

    for (int k = 0; k < p->N_zc; k++) {
      // This creates a 180-degree phase shift (Inverted signal)
      p->ifft_in[begin + k] = -1.0f * gain * sequence_data[k];
    }
